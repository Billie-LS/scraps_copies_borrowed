...
class MCSimulation:
    ...
    # Quasi random generator - Sobol
    def sobol(m, d = 1):
        sampler = qmc.Sobol(d, scrambler = True)
    ...
    """
    (mean_returns[s], std_returns[s])
    
    # Quasi random generator - Sobol
    def sobol(m=mean_returns[s], std_returns[s]):
        sampler = qmc.Sobol(std_returns[s], scrambler = True)
        return sampler.random_base2(m)
    
    def sobol_norm(m=mean_returns[s], std_returns[s]):
        sampler = qmc.Sobol(std_returns[s], scrambler = True)
        x_sobol = sampler.random_base2(m)
        return stats.norm.ppf(x_sobol)
    """ 
        ...
        for n in range(self.nSim):
            ...
            for s in range(len(last_prices)):
                ...
                for i in range(self.nTrading):
                    ...
                    simvals[s].append(simvals[s][-1] * (1 + sobol(m=9, d=2).T))
